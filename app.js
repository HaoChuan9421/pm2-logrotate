...
else if (process.env.HOME && !process.env.HOMEPATH) PM2_ROOT_PATH = path.resolve(process.env.HOME, ".pm2");
else if (process.env.HOME || process.env.HOMEPATH) PM2_ROOT_PATH = path.resolve(process.env.HOMEDRIVE, process.env.HOME || process.env.HOMEPATH, ".pm2");
...
function get_limit_size() {
  if (conf.max_size === "") return 1024 * 1024 * 256;
  if (typeof conf.max_size !== "string") conf.max_size = conf.max_size + "";
  if (conf.max_size.slice(-1) === "G") return parseInt(conf.max_size) * 1024 * 1024 * 1024;
  if (conf.max_size.slice(-1) === "M") return parseInt(conf.max_size) * 1024 * 1024;
...
function proceed_app(app, force) {
  ...
  _.uniq([app.pm2_env.pm_out_log_path, app.pm2_env.pm_err_log_path, app.pm2_env.pm_log_path]).forEach((file) => {
  ...
  setInterval(function () {
    ...
    pm2.list(function (err, apps) {
      ...
      apps.forEach(function (app) {
        ...
        if (app.pm2_env.exec_mode === "cluster_mode" && app.pm2_env.merge_logs) {
          if (appMap[app.name]) {
            return;
          } else {
            appMap[app.name] = app;
